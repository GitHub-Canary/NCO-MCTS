...
def value(x,A):
   ...
   for i in range(row):
     for j in range(column):
       ob=ob+x[i,j]*(A[i,j]+A[i,j+1]+A[i,j+2]+A[i+1,j]+A[i+1,j+1]+A[i+1,j+2]+A[i+2,j]+A[i+2,j+1]+A[i+2,j+2])
   return ob
...
row=18
column=18
x=Local.addVars(18,18,vtype=GRB.BINARY, name="x")  #addVars，多个变量记得加ss
Local.setObjective(value(x,A), GRB.MAXIMIZE)
for i in range(row-2):
  for j in range(column-2):
   Local.addConstr(x[i,j]+x[i,j+1]+x[i,j+2]+x[i+1,j]+x[i+1,j+1]+x[i+1,j+2]+x[i+2,j]+x[i+2,j+1]+x[i+2,j+2]<=1, "absconstr1")
